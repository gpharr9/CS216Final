...
#        self.print() # Prints out hands
    ...
    def get_count(self, display):
        self.p_count = self.player_instance.count_hand(False) # Checks total value of the Player hand
        
        if(display): # Checks total value of the Dealer hand
            self.d_count = self.dealer_instance.count_hand(True) 
        else:
            self.d_count = self.dealer_instance.count_hand(False)
    ...
    def make_stand(self):
        self.p_count = self.player_instance.count_hand(False) # Checks total value of the Player hand
        self.d_count = self.dealer_instance.count_hand(False) # Checks total value of the Dealer hand
    ...
    def condition(self, showAll):
        #print("Player Hand:") 
        p_arr = self.player_instance.print_hand() # Outputs the card layout based on player hand to the terminal

        #print("Dealer Hand:")
        d_arr = self.dealer_instance.print_hand(showAll) # Outputs the card layout based on dealer hand to the terminal
    ...
    def p_hit(self):
        ...
        self.p_count = self.player_instance.count_hand(False)
...
#            return state
    ...
    def validate_dealer(self): # Function to monitor dealer hand and ensure if follows game restrictions
        # check dealer count
        self.d_count = self.dealer_instance.count_hand(False) # Gathers dealers hand total value for the logic
