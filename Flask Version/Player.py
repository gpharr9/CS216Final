...
class Player:
  ...
  def count_hand(self, dealer): # Iterates through the hand to collect the value of all cards
    ...
    for i in range(s, len(x)): #Determines what the cards value will be if the nonsuit is a face card
      if(x[i] == 'j'):
        x[i] = 10
      elif (x[i] == 'a'):
        x[i] = 1
